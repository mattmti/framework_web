...
export const useGameStore = defineStore('game', {
  ...
  async startDailyGame() {
    ...
    try {
      ...
      this._resetGame('daily')  // Reset d'abord
      ...
      if (this.dailyStatus?.session?.attempts_list) {
        ...
        // Si la session était déjà gagnée ou terminée
        if (this.dailyStatus.session.won) {
          this.isWon = true
          this.isGameOver = true
          this.pointsEarned = this.dailyStatus.session.points_earned || 0
        } else if (this.attemptsUsed >= 10) {
          this.isGameOver = true
        }
      }
